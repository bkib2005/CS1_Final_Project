...
class RunningUtilsUnitTests(unittest.TestCase):
    ...
    def test_elevation_gain_004(self):
        elevations = [0, 20, 20, 40, 50, ]
        expected = 50
        actual = elevation_gain(elevations)
        self.assertTrue( RunningUtilsUnitTests.is_close(expected, actual, self.TOLERANCE) )

    def test_elevation_gain_005(self):
        elevations = [8, 8, 8, 8, 8]
        expected = 0
        actual = elevation_gain(elevations)
        self.assertTrue( RunningUtilsUnitTests.is_close(expected, actual, self.TOLERANCE) )
    ...
    def test_convert_map_004(self):
        map = [
          [ 0, 1, 1, 2, 3, 4 ],
        ]
        expected = [
          [ 0.0, 3.28084, 3.28084, 6.56168, 9.84252, 13.12336 ],
        ]
        actual = convert_map(map)
        self.assertTrue( RunningUtilsUnitTests.is_close_2d(expected, actual, self.TOLERANCE) )

    def test_convert_map_005(self):
        map = [
          [ 0, 5, 10, 15, 20, 25 ],
        ]
        expected = [
          [ 0.0, 16.4042, 32.8084, 49.2126, 65.6168, 82.021 ],
        ]
        actual = convert_map(map)
        self.assertTrue( RunningUtilsUnitTests.is_close_2d(expected, actual, self.TOLERANCE) )
